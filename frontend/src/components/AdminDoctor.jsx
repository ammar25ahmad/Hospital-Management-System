...
function AdminDoctor() {
  ...
  const handleAddDoctor = async (e) => {
    e.preventDefault();
        try{
      const response = await axios.post('http://localhost:3000/addDoctor', newDoctor)
  ...
  useEffect(() => {
    ...
    const fetchDoctors = async (query = '') => {
      try {
        ...
        const url = query ? `http://localhost:3000/fetchDoctor?q=${encodeURIComponent(query)}` : 'http://localhost:3000/fetchDoctor';
  ...
  return (
    <div className="p-6 bg-gray-50 min-h-screen">
      <div className="flex justify-between items-center mb-6">
        <h1 className="text-2xl font-semibold text-gray-800">List of doctors</h1>
        <button 
      ...
      <p className="text-sm text-gray-500 mb-6">{doctors.length || 0} available doctors</p>
              ...
              {doctors.length > 0 ? (
                doctors.map((doctor) => (
                  <tr key={doctor.id} className="hover:bg-gray-50">
                    <td className="px-6 py-4 whitespace-nowrap">
                      <div className="flex items-center">
                        <div className="flex-shrink-0 h-10 w-10">
                          <img className="h-10 w-10 rounded-full" src={doctor.avatar || "/user.png"} alt="" />
                        </div>
                        <div className="ml-4">
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">{doctor.gender}</td>
                ...
                <tr>
                  <td colSpan="7" className="px-6 py-4 text-center text-sm text-gray-500">No doctors found</td>
                </tr>
              )}
            </tbody>
          </table>
          ...
          <div className="bg-white rounded-lg shadow-lg max-w-lg w-full p-6 relative">
            ...
            <form onSubmit={handleAddDoctor} className="space-y-4">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                <div>
                  <label className="block text-gray-700 font-medium mb-1" htmlFor="cnic">CNIC</label>
                  <input
                    id="cnic"
                    type="text"
                    className="w-full border border-gray-300 rounded-md px-3 py-2 focus:outline-none focus:ring-2 focus:ring-blue-500"
                    value={newDoctor.cnic}
                    onChange={(e) => setNewDoctor({...newDoctor, cnic: e.target.value})}
                    required
                    name='cnic'
                  />
                </div>
