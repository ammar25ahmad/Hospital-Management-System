...
const verifyAdminToken = (req, res, next) => {
    ...
    if (authHeader) {
        ...
        jwt.verify(token, secret, (err, decoded) => {
            ...
            if (!decoded || decoded.role !== 'admin') { // Ensure the token signifies an admin user and is decoded
                return res.status(403).json({ message: 'Access denied. Admin privileges required or token invalid.' });
            }
